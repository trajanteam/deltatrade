...
contract SafeMath {
  ...
  function assertion(bool _assertion) internal {
    if (!_assertion) revert();
  }
}
...
contract ReserveToken is StandardToken, SafeMath {
  ...
  function create(address account, uint amount) public onlyMinter {
    balances[account] = safeAdd(balances[account], amount);
    totalSupply = safeAdd(totalSupply, amount);
  }

  function destroy(address account, uint amount) public onlyMinter {
...
contract DeltaTrade is SafeMath {
  ...
  function DeltaTrade(address admin_, address feeAccount_, uint feeMake_, uint feeTake_) public {
    admin = admin_;
  ...
  function withdraw(uint amount) public {
    ...
    // Is is right to write "require(tokens[0][msg.sender] >= amount);" ?
  ...
  function availableVolume(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint8 v, bytes32 r, bytes32 s) private view returns(uint) {
    bytes32 hash = sha256(this, tokenGet, amountGet, tokenGive, amountGive, expires, nonce);
    if (!(
